...
log "== role resource regression =="
ops_json="$(request_any GET "/roles?scope=ops&page=1&page_size=20" "200,403" '.' "roles-ops-list")"
request_any GET "/roles?scope=institution&page=1&page_size=20" "200,403,500" '.' "roles-inst-list"
request_any POST "/roles?scope=ops" "403,400" '.' "roles-create-ops-forbidden" '{"name":"x","code":"x"}'
role_id="$(echo "$ops_json" | jq -r '.items[0].id // empty' 2>/dev/null || true)"
if [[ -n "$role_id" ]]; then
  request_any GET "/roles/${role_id}?scope=ops" "200,403" '.' "role-detail"
  request_any GET "/roles/${role_id}/permissions?scope=ops" "200,403" '.' "role-permissions"
  request_any GET "/roles/${role_id}/menus?scope=ops" "200,403,400" '.' "role-menus"
fi
