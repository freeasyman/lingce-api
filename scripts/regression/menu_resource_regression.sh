...
log "== menu resource regression =="
ops_json="$(request_any GET "/menus?scope=ops" "200,403" '.' "menus-ops-list")"
request_any GET "/menus?scope=institution" "200,403" '.' "menus-inst-list"
request_any GET "/menus/tree?scope=ops" "200,403" '.' "menus-tree-ops"
request_any PUT "/menus/sort?scope=ops" "403,400" '.' "menus-sort-forbidden" '{"menu_ids":[]}'
menu_id="$(echo "$ops_json" | jq -r '.items[0].id // empty' 2>/dev/null || true)"
if [[ -n "$menu_id" ]]; then request_any GET "/menus/${menu_id}?scope=ops" "200,403" '.' "menu-detail"; fi
