...
resp="$(request GET "/api/v1/recordings?page=1&page_size=5&tenant_id=$TENANT_ID" "")"
assert_status "200" "${resp%%|*}" "GET /api/v1/recordings"
resp="$(request GET "/api/v1/recordings/quality-control?tenant_id=$TENANT_ID" "")"
assert_status "200" "${resp%%|*}" "GET /api/v1/recordings/quality-control"
resp="$(request GET "/api/v1/recordings/communication-analysis?tenant_id=$TENANT_ID" "")"
assert_status "200" "${resp%%|*}" "GET /api/v1/recordings/communication-analysis"

log "advanced actions"
resp="$(request POST "/api/v1/recordings/$RECORDING_ID/actions/transcribe" "{}")"
transcribe_status="${resp%%|*}"
assert_any_status "$transcribe_status" "POST /api/v1/recordings/{id}/actions/transcribe" "200" "500"
resp="$(request POST "/api/v1/recordings/$RECORDING_ID/actions/analyze" "{}")"
analyze_status="${resp%%|*}"
assert_any_status "$analyze_status" "POST /api/v1/recordings/{id}/actions/analyze" "200" "500"
resp="$(request POST "/api/v1/recordings/$RECORDING_ID/actions/clean" "{}")"
clean_status="${resp%%|*}"
assert_any_status "$clean_status" "POST /api/v1/recordings/{id}/actions/clean" "200" "500"
...
resp="$(request POST "/api/v1/recordings/actions/batch-delete" "{\"recording_ids\":[]}")"
bad_batch_status="${resp%%|*}"
assert_status "400" "$bad_batch_status" "POST /api/v1/recordings/actions/batch-delete with empty ids"
