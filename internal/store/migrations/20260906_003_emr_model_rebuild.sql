-- EMR development rebuild. The former EMR tables contain only development
-- data and are intentionally recreated from the confirmed target models.

CREATE EXTENSION IF NOT EXISTS pgcrypto;

-- Remove the former candidate-generation configuration. The unified generation
-- contract is introduced by the later input/generation implementation.
DELETE FROM recording_analysis_prompts
WHERE code = 'emr_candidate_generation_v1';
...
DROP TABLE IF EXISTS emr_ai_generations CASCADE;
...
CREATE UNIQUE INDEX emr_records_tenant_encounter_uq ON emr_records (tenant_id, encounter_id);
...
CREATE TABLE emr_ai_generations (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  record_id UUID NOT NULL REFERENCES emr_records(id) ON DELETE CASCADE,
  generation_key VARCHAR(255) NOT NULL,
  input_basis JSONB NOT NULL DEFAULT '{}'::jsonb,
  section_updates JSONB NOT NULL DEFAULT '{}'::jsonb,
  source_evidence JSONB NOT NULL DEFAULT '{}'::jsonb,
  generated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  model_call_reference JSONB NOT NULL DEFAULT '{}'::jsonb
);
CREATE UNIQUE INDEX emr_ai_generations_record_key_uq ON emr_ai_generations (record_id, generation_key);
CREATE INDEX emr_ai_generations_record_time_idx ON emr_ai_generations (record_id, generated_at DESC);
...
CREATE TABLE emr_process_records (
  ...
  CONSTRAINT emr_process_action_result_check CHECK (action_result IN ('成功', '失败', '阻断')),
  CONSTRAINT emr_process_actor_type_check CHECK (actor_type IN ('人工', '系统')),
  CONSTRAINT emr_process_actor_check CHECK (actor_type = '系统' OR actor_id IS NOT NULL),
  CONSTRAINT emr_process_failure_reason_check CHECK (action_result = '成功' OR NULLIF(BTRIM(failure_reason), '') IS NOT NULL),
  CONSTRAINT emr_process_record_reference_check CHECK (
    record_id IS NOT NULL OR action_type = '机构范围过程记录导出' OR (action_type = '创建' AND action_result = '失败')
  )
);
